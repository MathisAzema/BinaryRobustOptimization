struct ThermalUnitMixedRecourse2
    name::Int64
    Bus::Int64
    MinPower::Float64  
    MaxPower::Float64  
...
struct UnitCommitmentMixedRecourse2 <: AbstractProblem
    name::String
    T::Int64
    Nslow::Int64
    Nfast::Int64
    slowunits::Vector{ThermalUnitMixedRecourse2}
    fastunits::Vector{ThermalUnitMixedRecourse2}
    Buses::Int64
    Lines::Vector{LineMixedRecourse2}
    ...
    # function UnitCommitmentMixedRecourse2(filename::String, budget::Int, N1::Int)
    ...
    #     slowunits=Vector{ThermalUnitMixedRecourse2}(undef, N1)
    #     fastunits=Vector{ThermalUnitMixedRecourse2}(undef, N-N1)
    #     k=0
    ...
    #                     unit=ThermalUnitMixedRecourse2(unit_name, Bus, MinPower, MaxPower, DeltaRampUp, DeltaRampDown, QuadTerm, StartUpCost, StartDownCost, LinearTerm, ConstTerm, InitialPower, InitUpDownTime, Int64(MinUpTime), Int64(MinDownTime))
    #                     if k <= N1
    ...
    function UnitCommitmentMixedRecourse2(folder::String, budget::Int, N1::Int)
        """
        ...
        for i in 2:NumberUnits+1
            ...
            unit=ThermalUnitMixedRecourse2(unit_name, Bus, MinPower, MaxPower, DeltaRampUp, DeltaRampDown, QuadTerm, StartUpCost, StartDownCost, LinearTerm, ConstTerm, InitialPower, InitUpDownTime, MinUpTime, MinDownTime)
            if i-1 <= N1
                slowunits[unit_name]=unit
            else
                fastunits[unit_name]=unit
        ...
        Lines=Vector{LineMixedRecourse2}(undef, Numlines)
        for i in 2:Numlines+1
            ...
            Lines[i-1]=LineMixedRecourse2(id, b1, b2, fmax, X)
        end
...
mixed_integer_recourse(UC::UnitCommitmentMixedRecourse2) = true

complete_recourse(UC::UnitCommitmentMixedRecourse2) = true

objective_scale(UC::UnitCommitmentMixedRecourse2) = 1.0

indicator_uncertainty(UC::UnitCommitmentMixedRecourse2) = false

function solve_deterministic_problem(UC::UnitCommitmentMixedRecourse2)
...
function init_master(UC::UnitCommitmentMixedRecourse2)
    m = initializeJuMPModel()
...
function update_master_mixed_integer(UC::UnitCommitmentMixedRecourse2, MP_outer::JuMP.Model, MP_inner::JuMP.Model, master::MasterType)
    s = MP_outer[:s]
...
function init_master_inner_level(UC::UnitCommitmentMixedRecourse2, master_inner::SubproblemType)
    m = initializeJuMPModel()
    # uncertainty set
...
function init_master_inner_level(UC::UnitCommitmentMixedRecourse2, MP_inner::JuMP.Model)
...
function update_master_inner_level(UC::UnitCommitmentMixedRecourse2, MP_outer::JuMP.Model, MP_inner::JuMP.Model, SP_inner::JuMP.Model, master_inner::SubproblemType, λ = nothing)
...
function update_master_inner_level(UC::UnitCommitmentMixedRecourse2, MP_inner::JuMP.Model, power_slow, is_on_slow, start_up_slow, start_down_slow, is_on_fast, start_up_fast, start_down_fast, master_inner::SubproblemType, λ = nothing)
...
function compute_lagrangian_coefficient(UC::UnitCommitmentMixedRecourse2, MP_outer::JuMP.Model)
    return sum(sum(UC.DemandDev))*UC.PenaltyCost/15
end

function build_second_stage_problem(UC::UnitCommitmentMixedRecourse2, MP_outer::JuMP.Model, MP_inner::JuMP.Model)
    is_on_slow = JuMP.value.(MP_outer[:is_on_slow])
    is_on_slow = Float64.(Int.(round.(is_on_slow)))
    start_up_slow = JuMP.value.(MP_outer[:start_up_slow])
    start_up_slow = Float64.(Int.(round.(start_up_slow)))
...
function solve_second_stage_problem_lagrangian(UC::UnitCommitmentMixedRecourse2, MP_outer::JuMP.Model, MP_inner::JuMP.Model, λ::Float64)
    is_on_slow = JuMP.value.(MP_outer[:is_on_slow])
    is_on_slow = Float64.(Int.(round.(is_on_slow)))
    start_up_slow = JuMP.value.(MP_outer[:start_up_slow])
    start_up_slow = Float64.(Int.(round.(start_up_slow)))
...
function record_discrete_second_stage_decision(UC::UnitCommitmentMixedRecourse2, SP_inner::JuMP.Model, discrete_decision_list::Dict)
    is_on_fast = JuMP.value.(SP_inner[:is_on_fast])
    is_on_fast = Float64.(Int.(round.(is_on_fast)))
...
function record_scenario(UC::UnitCommitmentMixedRecourse2, MP_inner::JuMP.Model, scenario_list::Dict)
    ξ = JuMP.value.(MP_inner[:ξ])
...
function solve_MP_inner_enumeration(UC::UnitCommitmentMixedRecourse2, MP_outer::JuMP.Model, MP_inner::JuMP.Model)
